...
NATIVE_LANGUAGE = 'en'
file = File.open('./ruby_rating.txt', 'a')
file2 = File.open('./ruby_review.txt', 'a')

# MODIFY THIS HASH WITH YOUR APP SET (grab the itunes store urls & pull the id params)
software = {
  # https://itunes.apple.com/us/app/chase-mobile/id298867247?mt=8
  # 'Chase' => 298867247,
  # 'BoA' => 284847138,
  # 'Wells' => 311548709,
...
def fetch_reviews(software_id, store, numPages)
  reviews = []
	# numberThing = 0 #{numberThing}
  ...
  cmd = sprintf(%{curl -s -A "iTunes/9.2 (Macintosh; U; Mac OS X 10.6" -H "X-Apple-Store-Front: %s-1" } <<
                ...
                %{pageNumber=#{numPages}&sortOrdering=1&type=Purple+Software' | xmllint --format --recover - 2>/dev/null},
                store[:id],
                software_id)
  rawxml = `#{cmd}`
	puts("Store ID: #{store[:id]}")
	puts("Software ID: #{software_id}")
	puts(rawxml)
...
# file = File.open('./ruby_data.txt', 'a')
software.keys.sort.each do |software_key|
  ...
  stores.sort_by { |a| a[:name] }.each do |store|
		...
		$num = 10
		while $i < $num do
			reviews = fetch_reviews(software[software_key], store, $i)
			$i += 1
			puts("Loop number: #{$i}")
    ...
    if reviews.any?
      puts "=== Store: #{store[:name]}"
      	reviews.each_with_index do |review, index|
					# file.write("#{review[:rating]},#{review[:date]},#{review[:body]}\t")
					file2.write("#{review[:body]}\t")
